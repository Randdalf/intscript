...
class IntcodeTests(unittest.TestCase):
    ...
    def memory_test(slf, program, expected):
        """After execution, check that the memory is as expected."""
        cpu = intcode(slf.intcode, len(program), *program)
        actual = cpu.memory[slf.memstart:slf.memstart+len(program)]
        slf.assertEqual(actual, expected)
